...
namespace Exam_System
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Subject subject = new Subject(subjectId, subjectName, new Exam(numOfQuestions, TimeSpan.FromMinutes(timeInMinutes)));
            ...
            for (int i = 0; i < numOfQuestions; i++)
            {
                ...
                if (type == 2)
                {
                    ...
                    if (Qtype == 1)
                    {
                        ...
                        questions[i] = new Question(1, questionBody, questionMark, answers, answers[rightAnswerId - 1]);
                    }
                    else if (Qtype == 2)
                    {
                        ...
                        int rightAnswerId = Check_int(Console.ReadLine());
                        questions[i] = new Question(2, questionBody, questionMark, answers, answers[rightAnswerId-1]);
                    }
                }
                else
                {
            ...
            if (startExam.ToLower() == "y"||startExam=="1")
            {
                ...
                if (type == 2)
                {
                    subject.CreateFinalExam();
                    Console.WriteLine("--------------------");
                }
                else
                    subject.CreatePracticalExam(); Console.WriteLine("--------------------");
                ...
                if (type == 2)
                {
                    FinalExam finalExam = new FinalExam(questions);
                    finalExam.DisplayRightAnswers();
                    print($"Your total marks is: {totalMarks}/{finalExam.CalculateTotalMarks()}");
                }
                else
                {
                    PricticalExam practicalExam = new PricticalExam(questions);
                    practicalExam.DisplayRightAnswers();
                }
